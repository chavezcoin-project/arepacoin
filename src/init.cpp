...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, arepacoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
...
//    strUsage += "  -anonymizearepacoinamount=<n>     " + strprintf(_("Keep N ARC anonymized (default: %u)"), 0) + "\n";
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // ArepaCoin: load previous sessions sporks if we have them.
                ...
                // ArepaCoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    LogPrintf(">> Dumping Masternode assets\n");
...
// //    }
// //
// //    nAnonymizePivxAmount = GetArg("-anonymizearepacoinamount", 0);
