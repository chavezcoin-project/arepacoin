...
Value mintzerocoin(const Array& params, bool fHelp)
{
    ...
    if (GetAdjustedTime() < GetSporkValue(SPORK_17_ENABLE_ZEROCOIN))
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: Zerocoin functionality is not enabled on the ArepaCoin network yet.");
...
Value spendzerocoin(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zARC (zerocoins) into ARC. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over ARC (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zARC
